...
struct Vector3f {
	...
	bool is_equal(const Vector3f& v) const {
		if (abs(x - v.x) < 1e-6)
		{
			if (abs(y - v.y) < 1e-6)
			{
...
public:
	...
	inline Vector3f operator*(const Vector3f& v) const {
		...
		r.x = m[0][0] * v.x + m[0][1] * v.y + m[0][2] * v.z + m[0][3] * 1.0;
		r.y = m[1][0] * v.x + m[1][1] * v.y + m[1][2] * v.z + m[1][3] * 1.0;
		r.z = m[2][0] * v.x + m[2][1] * v.y + m[2][2] * v.z + m[2][3] * 1.0;
		//r.w = m[3][0] * v.x + m[3][1] * v.y + m[3][2] * v.z + m[3][3] * v.w;
...
public:
	...
	inline Vector3f operator*(const Vector3f& v) const {
		...
		//r.w = m[3][0] * v.x + m[3][1] * v.y + m[3][2] * v.z;
	...
	Matrix3f& Inverse() {
		...
		Matrix3f res;
		res.m[0][0] = invdet * (m[1][1] * m[2][2] - m[1][2] * m[2][1]);
		res.m[0][1] = invdet * (m[0][2] * m[2][1] - m[2][2] * m[0][1]);
		res.m[0][2] = invdet * (m[0][1] * m[1][2] - m[0][2] * m[1][1]);

		res.m[1][0] = invdet * (m[1][2] * m[2][0] - m[1][0] * m[2][2]);
		res.m[1][1] = invdet * (m[0][0] * m[2][2] - m[0][2] * m[2][0]);
		res.m[1][2] = invdet * (m[0][2] * m[1][0] - m[1][2] * m[0][0]);

		res.m[2][0] = invdet * (m[1][0] * m[2][1] - m[2][0] * m[1][1]);
		res.m[2][1] = invdet * (m[0][1] * m[2][0] - m[2][1] * m[0][0]);
		res.m[2][2] = invdet * (m[0][0] * m[1][1] - m[0][1] * m[1][0]);
...
public:
	...
	inline Vector2f operator*(const Vector2f& v) const {
		...
		r.x = m[0][0] * v.x + m[0][1] * v.y;
		r.y = m[1][0] * v.x + m[1][1] * v.y;
		//r.w = m[3][0] * v.x + m[3][1] * v.y + m[3][2] * v.z;
	...
	Matrix2f& Inverse() {
		...
		Matrix2f res;
		res.m[0][0] = invdet * m[1][1];
		res.m[0][1] = invdet * (- m[0][1]);

		res.m[1][0] = invdet * (- m[1][0]);
		res.m[1][1] = invdet * (m[0][0]);
